class Solution {
public:
    void make_combo(int i, vector<int> &nums, int sum, int &target, vector<int> &temp, set<vector<int>> &ans) {
        ...
        // consider and move ahead
        temp.push_back(nums[i]);
        make_combo(i+1, nums, sum+nums[i], target, temp, ans);
        temp.pop_back();
        // consider and stay
        temp.push_back(nums[i]);
